...
namespace Boxey.Core {
    public class Chunk {
        ...
        private bool _chunkMeshGenerated;
        ...
        public void Update() {
            if (!_chunkMeshGenerated) return;
            var distance = Vector3.Distance(ChunkObject.transform.position, _camera.transform.position);
            if (distance > _lodData.viewDistance) return;
            ChunkObject.SetActive(true);
            _chunkCollider.enabled = _lodData.canCollideWith;
        }
        #endregion
        ...
        public MeshFilter GetFilter() {
            return _chunkMeshGenerated ? _chunkFilter : null;
        }
        private void BuildMesh() {
            _chunkMeshGenerated = false;
            ChunkObject.SetActive(_chunkMeshGenerated);
