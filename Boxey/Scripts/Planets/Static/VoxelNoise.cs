...
namespace Boxey.Scripts.Planets.Static {
    public static class VoxelNoise {
        //3D Noise Functions
        private static float[,,] Get3DNoiseMap(int width, int height, int depth, PlanetSettings settings) {
        ...
        private static float[,,] Get3DNoiseMapJob(int width, int height, int depth, PlanetSettings settings) {
            float[,,] map3D = new float[width + 1, height + 1, depth + 1];
        ...
        public static float[,,] GetPlanetNoiseMap(int radius, int additive, PlanetSettings settings) {
            var diameter = (radius) * 2;
            var center = Vector3.one * radius;
            var noiseMap = Get3DNoiseMap(diameter, diameter, diameter, settings);
        ...
        public static float[,,] GetPlanetNoiseMapJob(int radius, int additive, PlanetSettings settings) {
            var diameter = (radius) * 2;
            var map3D = new float[diameter + 1, diameter + 1, diameter + 1];
            var noiseMap = Get3DNoiseMapJob(diameter, diameter, diameter, settings);
            ...
            var handle = job.Schedule(values.Length, diameter + 1);
            handle.Complete();
            int index = 0;
            for (int x = 0; x <= diameter; x++) {
                for (int y = 0; y <= diameter; y++) {
